    ...
    public void rightClickMouse()
    {
        if (!this.playerController.getIsHittingBlock())
        {
            ...
            else
            {
                ...
                {
                    case ENTITY:
                        if (this.playerController.isPlayerRightClickingOnEntity(this.player, this.objectMouseOver.entityHit, this.objectMouseOver))
                        {
                            flag = false;
                        }
                ...
                if (Keyboard.getEventKeyState())
                {
                    ...
                    if (currentScreen == null) {
                        if (k == Keyboard.KEY_RSHIFT) {
                            displayGuiScreen(Mania.getClickGui());
                        }

                        Mania.getModuleManager().getModules().stream()
                                .filter(m -> m.keyCode == k)
                                .forEach(Module::toggle);
                    }

                    final KeyPressEvent a = new KeyPressEvent(k);
                    Mania.getEventManager().call(a);
