...
public class EntityRenderer implements IResourceManagerReloadListener
{
    ...
    public void getMouseOver(float partialTicks)
    {
        ...
        if (entity != null && this.mc.world != null)
        {
            ...
            Vec3 vec33 = null;
            float f = 1.0F;
            List<Entity> list = this.mc.world.getEntitiesInAABBexcluding(entity, entity.getEntityBoundingBox().addCoord(vec31.xCoord * d0, vec31.yCoord * d0, vec31.zCoord * d0).expand((double)f, (double)f, (double)f), Predicates.and(EntitySelectors.NOT_SPECTATING, new Predicate<Entity>()
            {
    ...
    private void orientCamera(float partialTicks)
    {
        ...
        if (entity instanceof EntityLivingBase && ((EntityLivingBase)entity).isPlayerSleeping())
        {
            ...
            if (!this.mc.gameSettings.debugCamEnable)
            {
                ...
                GlStateManager.rotate(RotationUtil.virtualPrevYaw + (RotationUtil.virtualYaw - RotationUtil.virtualPrevYaw) * partialTicks + 180.0F, 0.0F, -1.0F, 0.0F);
                GlStateManager.rotate(RotationUtil.virtualPrevPitch + (RotationUtil.virtualPitch - RotationUtil.virtualPrevPitch) * partialTicks, -1.0F, 0.0F, 0.0F);
            }
        }
        else if (this.mc.gameSettings.thirdPersonView > 0)
        {
            ...
            else
            {
                ...
                float f2 = RotationUtil.virtualPitch;
                ...
                for (int i = 0; i < 8; ++i)
                {
                    ...
                    f4 = f4 * 0.1F;
                    f5 = f5 * 0.1F;
                    MovingObjectPosition movingobjectposition = this.mc.world.rayTraceBlocks(new Vec3(d0 + (double)f3, d1 + (double)f4, d2 + (double)f5), new Vec3(d0 - d4 + (double)f3 + (double)f5, d1 - d6 + (double)f4, d2 - d5 + (double)f5));
                ...
                GlStateManager.rotate(RotationUtil.virtualPitch - f2, 1.0F, 0.0F, 0.0F);
                GlStateManager.rotate(RotationUtil.virtualYaw - f1, 0.0F, 1.0F, 0.0F);
                GlStateManager.translate(0.0F, 0.0F, (float)(-d3));
                GlStateManager.rotate(f1 - RotationUtil.virtualYaw, 0.0F, 1.0F, 0.0F);
                GlStateManager.rotate(f2 - RotationUtil.virtualPitch, 1.0F, 0.0F, 0.0F);
            }
        }
        else
        {
        ...
        if (Reflector.EntityViewRenderEvent_CameraSetup_Constructor.exists())
        {
            if (!this.mc.gameSettings.debugCamEnable)
            {
                float f6 = RotationUtil.virtualPrevYaw + (RotationUtil.virtualYaw - RotationUtil.virtualPrevYaw) * partialTicks + 180.0F;
                float f7 = RotationUtil.virtualPrevPitch + (RotationUtil.virtualPitch - RotationUtil.virtualPrevPitch) * partialTicks;
                float f8 = 0.0F;
    ...
    private void updateLightmap(float partialTicks)
    {
        if (this.lightmapUpdateNeeded)
        {
            ...
            if (world != null)
            {
                if (Config.isCustomColors() && CustomColors.updateLightmap(world, this.torchFlickerX, this.lightmapColors, this.mc.player.isPotionActive(Potion.nightVision), partialTicks))
                {
